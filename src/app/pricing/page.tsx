        ...
        <div className="container px-4 md:px-6">
          ...
          >
            ...
            <p className={`mt-4 mx-auto max-w-[700px] text-lg ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
              Choose the plan that's right for you and start managing your events with ease.
            </p>
          </motion.div>
          <div className="grid gap-8 md:grid-cols-3">
            {plans.map((plan, index) => (
              ...
              >
                <h2 className={`text-2xl font-bold mb-4 ${isDarkMode ? 'text-gray-100' : 'text-gray-900'}`}>{plan.name}</h2>
                <p className={`text-4xl font-bold mb-6 ${isDarkMode ? 'text-indigo-400' : 'text-indigo-600'}`}>{plan.price}</p>
                <ul className="mb-6 flex-1">
                  {plan.features.map((feature, featureIndex) => (
                    <li key={featureIndex} className="flex items-center mb-2">
                      <Check className={`mr-2 h-5 w-5 ${isDarkMode ? 'text-green-400' : 'text-green-500'}`} />
