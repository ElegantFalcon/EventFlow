...
import { Calendar, Moon, Sun, Mail, Lock, User, ArrowRight } from "lucide-react"
    ...
    <div className={`flex flex-col min-h-screen ${
      ...
      <main className="flex-1 flex items-center justify-center p-4">
        <motion.div
          ...
          className={`w-full max-w-md p-8 rounded-lg shadow-lg ${isDarkMode ? 'bg-gray-800/50' : 'bg-white/50'} backdrop-blur-md`}
        >
          <h1 className={`text-2xl font-bold text-center mb-6 ${isDarkMode ? 'text-gray-100' : 'text-gray-900'}`}>
          ...
          <form className="space-y-4">
            ...
            >
              <div className="flex items-center space-x-2">
                ...
                <Label htmlFor="user" className={isDarkMode ? 'text-gray-300' : 'text-gray-700'}>User</Label>
              </div>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="admin" id="admin" />
                <Label htmlFor="admin" className={isDarkMode ? 'text-gray-300' : 'text-gray-700'}>Admin</Label>
              </div>
            </RadioGroup>
            {!isLogin && (
              <div className="space-y-2">
                ...
                <div className="relative">
                  <User className={`absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`} />
                  <Input
                    id="name"
                    type="text"
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Mail className={`absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`} />
                <Input
                  id="email"
                  type="email"
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Lock className={`absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`} />
                <Input
                  id="password"
                  type="password"
            ...
            <Button
              type="submit"
              className={`w-full ${isDarkMode ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-indigo-600 hover:bg-indigo-700 text-white'}`}
            >
